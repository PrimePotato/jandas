package com.lchclearnet.table.joining;
...
import com.lchclearnet.table.index.*;
    ...
    private Table joinInternal(Table table1, Table table2, boolean outer, boolean allowDuplicates, String... col2Names) {
        ...
        for (Row row : table1) {
            ...
            for (int i = 0; i < columns.length; i++) {
                ...
                // same table, so use the original column's name to getObject the corresponding
                ...
                if (type instanceof DateColumnType) {
                    ...
                    rowBitMapOneCol = index.get(value);
                } else if (type instanceof CurrencyColumnType) {
                    IntIndex index = new IntIndex(table2.currencyColumn(col2Name));
                    CurrencyColumn col1 = (CurrencyColumn) table1Column;
                    int value = col1.getIntInternal(ri);
                    rowBitMapOneCol = index.get(value);
                } else if (type instanceof CurrencyPairColumnType) {
                    IntIndex index = new IntIndex(table2.currencyPairColumn(col2Name));
                    CurrencyPairColumn col1 = (CurrencyPairColumn) table1Column;
                    int value = col1.getIntInternal(ri);
                    rowBitMapOneCol = index.get(value);
                } else if (type instanceof EnumColumnType) {
                    IntIndex index = new IntIndex(table2.enumColumn(col2Name));
                    EnumColumn col1 = (EnumColumn) table1Column;
                    int value = col1.getIntInternal(ri);
                    rowBitMapOneCol = index.get(value);
                } else {

...
package com.lchclearnet.table;

import com.google.common.base.Preconditions;
import com.google.common.primitives.Ints;
import com.lchclearnet.table.aggregate.AggregateFunction;
import com.lchclearnet.table.aggregate.CrossTab;
import com.lchclearnet.table.aggregate.Summarizer;
...
import com.lchclearnet.table.io.DataFrameWriter;
import com.lchclearnet.table.io.html.HtmlTableWriter;
import com.lchclearnet.table.joining.DataFrameJoiner;
import com.lchclearnet.table.selection.BitmapBackedSelection;
import com.lchclearnet.table.selection.Selection;
import com.lchclearnet.table.sorting.Sort;
import com.lchclearnet.table.sorting.SortUtils;
import com.lchclearnet.table.sorting.comparators.IntComparatorChain;
import com.lchclearnet.table.table.Relation;
import com.lchclearnet.table.table.Rows;
import com.lchclearnet.table.table.StandardTableSliceGroup;
import com.lchclearnet.table.table.TableSliceGroup;
...
import static com.lchclearnet.table.aggregate.AggregateFunctions.countMissing;
import static com.lchclearnet.table.selection.Selection.selectNRowsAtRandom;

/**
 * A table of jandas, consisting of some number of columns, each of which has the same number of rows.
 * All the jandas in a column has the same type: integer, float, category, etc., but a table may contain an arbitrary
 * number of columns of any type.
 * <p>
 * Tables are the main jandas-type and primary focus of Airframe.
 */
public class Table extends Relation implements Iterable<Row> {

    /**
     * The columns that hold the jandas in this table
     */
     ...
     * @param columns One or more columns, all of which must have either the same rowCount or size 0
     */
    ...
    public <T> Table addColumn(final String columnName, Class<T> columnType, Function<Row, T> cellFunction) {
        return addColumn(columnName, ColumnFactory.type(columnType), cellFunction);
    }

    public <T> Table addColumn(final String columnName, String columnType, Function<Row, T> cellFunction) {
        Column<T> column = ColumnFactory.createColumn(columnName, columnType);
        for(Row row: this){
            column.append(cellFunction.apply(row));
        }
        insertColumn(columnList.size(), column);
        return this;
    }
    ...
    /**
     * Returns a table with the same columns as this table, but no jandas
     */
    ...
    /**
     * Returns a table with the same columns as this table, but no jandas, initialized to the given row size
     */
    ...
    /**
     * Clears all the jandas from this table
     */
    @Override
    public void clear() {
    ...
    /**
     * Returns a new table containing the first {@code nrows} of jandas in this table
     */
    ...
    /**
     * Returns a new table containing the last {@code nrows} of jandas in this table
     */
    ...
    public Table sortOn(String... columnNames) {
        ...
        for (String columnName : columnNames) {
            ...
            if (names.contains(columnName.toUpperCase())) {
                ...
                // getObject the prefix which could be - or +
    ...
    public boolean detect(Predicate<Row> predicate) {
        return first(predicate).isPresent();
    }

    public Optional<Row> first(Predicate<Row> predicate) {
        Row row = new Row(this);
        while (row.hasNext()) {
    ...
    public Optional<Row> last(Predicate<Row> predicate) {
        Row row = new Row(this);
        int offset = -1;
        while (row.hasNext()) {
            if (predicate.test(row.next())) {
                offset = row.getRowNumber();
            }
        }

        return offset == -1 ? Optional.empty(): Optional.of(row.__offset(offset));
    }

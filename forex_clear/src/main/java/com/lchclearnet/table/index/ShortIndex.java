...
public class ShortIndex {
    ...
    public Selection atMost(short value) {
        Selection selection = new BitmapBackedSelection();
        Short2ObjectSortedMap<IntArrayList> head = index.headMap((short) (value + 1));  // we add 1 to getObject values equal to the arg
    ...
    public Selection lessThan(short value) {
        Selection selection = new BitmapBackedSelection();
        Short2ObjectSortedMap<IntArrayList> head = index.headMap(value);  // we add 1 to getObject values equal to the arg
        for (IntArrayList keys : head.values()) {
            addAllToSelection(keys, selection);
        }
        return selection;

...
public class DataFrameMarketServiceFactory implements MarketServiceFactory {
    ...
    public <T extends MarketService> T getMarketSource(SourceType type,
        ...
        switch (type) {
            case FX_SPOT:
                mktData = serviceContext.computeIfAbsent(SourceType.FX_SPOT, x -> new DataFrameSpots((DataFrame<Integer, String>)dataContext.get(SourceType.FX_SPOT)));
                break;
            case FX_FORWARD:
                mktData = serviceContext.computeIfAbsent(SourceType.FX_FORWARD, x -> new DataFrameFxForwardFromYieldSourceService((DataFrame<Integer, String>)dataContext.get(SourceType.IR_YIELD_CURVE)));
                break;
            case IR_YIELD_CURVE:
                mktData = serviceContext.computeIfAbsent(SourceType.IR_YIELD_CURVE, x -> new DataFrameYieldCurveService((DataFrame<Integer, String>)dataContext.get(SourceType.IR_YIELD_CURVE)));
                break;
            case FX_VOLATILITY:
                mktData = null;
                break;
    ...
    public <T extends MarketService> T getMarketData(DataType type,
        ...
        switch (type) {
            case IR_YIELD_CURVE:
                mktData = serviceContext.computeIfAbsent(DataType.IR_YIELD_CURVE, x -> new DataFrameYieldCurveService((DataFrame<Integer, String>)dataContext.get(SourceType.IR_YIELD_CURVE)));
                break;
            case FX_SPOT:
                mktData = serviceContext.computeIfAbsent(DataType.FX_SPOT, x -> new DataFrameSpots((DataFrame<Integer, String>)dataContext.get(SourceType.FX_SPOT)));
                break;

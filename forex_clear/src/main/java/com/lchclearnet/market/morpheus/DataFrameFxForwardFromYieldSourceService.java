...
public class DataFrameFxForwardFromYieldSourceService extends AbstractDataFrameMarketService implements FxForwardSourceService {

    private final Map<String, YieldCurve> yieldCurves;

    private final Map<CurrencyPair, Map<LocalDate, Double>> fxFwdRates;

    public DataFrameFxForwardFromYieldSourceService(DataFrame<Integer, String> data) {
    ...
    @Override
    public Stream<Double> getForwardRates(final CurrencyPair ccyPair, final Double fxSpot, LocalDate... dates) {
        YieldCurve fyc = yieldCurves.computeIfAbsent(ccyPair.getForeign().getFxForwardYieldCurve(), curve -> buildYieldCurve(curve, data));
        YieldCurve dyc = yieldCurves.computeIfAbsent(ccyPair.getDomestic().getFxForwardYieldCurve(), curve -> buildYieldCurve(curve, data));

        return Streams.zip(fyc.discountFactors(dates), dyc.discountFactors(dates), (dff, dfd) -> fxSpot * dfd / dff);
    }

    /**
     * Retrieve the FX Forward Rate if it is cached, null otherwise.
     * The currency pair passed has to be in convention order.
     *
    ...
    private Double _getForwardRate(CurrencyPair conventionPair, LocalDate date) {
        if (!fxFwdRates.containsKey(conventionPair)) return null;
